...
export class SceneFinJeu extends Phaser.Scene {
    ...
    create() {
        ...
        this.texteFinJeu = this.add.text(this.game.config.width / 2, this.game.config.height / 5, "Fin de la partie", styleCommun);
        this.texteFinJeu.setOrigin(0.5);
        this.texteFinJeu.setColor("#ff0000");
        //ajouter le texte du score
        this.texteScore = this.add.text(this.game.config.width / 2, this.game.config.height / 2, `Score : ${this.leScore}`, styleCommun);
        this.texteScore.setOrigin(0.5);
        //ajouter le texte du meilleur score
        this.texteMeilleurScore = this.add.text(this.game.config.width / 2, this.game.config.height / 2.5, `Meilleur score : ${this.leMeilleurScore}`, styleCommun);
        this.texteMeilleurScore.setOrigin(0.5);
        //ajouter le texte qui dit au joueur qu'il peut rejouer
        this.texteRejouer = this.add.text(this.game.config.width / 2, this.game.config.height / 1.4, "Touchez n'importe \n où \n pour recommencer", styleCommun);
        this.texteRejouer.setOrigin(0.5);
        //réajuster la taille du texte personnalisé
        this.texteRejouer.setFontSize(Math.round(40 * GrilleMontage.ajusterRatioX()));
