...
export class SceneIntro extends Phaser.Scene {
  ...
  texteTitreJeu: Phaser.GameObjects.Text;

  texteInfoJeu: Phaser.GameObjects.Text;

  texteJouerJeu: Phaser.GameObjects.Text;
  ...
  create() {
    ...
    this.texteTitreJeu = this.add.text(
    ...
    this.texteInfoJeu = this.add.text(
      this.game.config.width / 2,
      this.game.config.height / 2.50,
      "Bougez de gauche a droite pour éviter les obstacles dans le donjon ! \n \n Flèches du clavier & 'swipe' sur mobile pour bouger",
      styleCommun
    );
    ...
    this.texteJouerJeu = this.add.text(
      this.game.config.width / 2,
